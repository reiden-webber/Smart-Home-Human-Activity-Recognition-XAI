...
class BiLSTMExperiment(Experiment):
    ...
    def build_model_classifier(self, nb_features, run_number=0):
        ...
        # classifier
        feature_1 = Input(shape=((nb_timesteps,nb_features)))
        masked = Masking(mask_value=0)(feature_1)
        lstm_1 = Bidirectional(LSTM(output_dim))(masked)

        output_layer = Dense(nb_classes, activation='softmax')(lstm_1)

        self.classifier_model = Model(inputs=feature_1, outputs=output_layer, name="BiLSTM_No_Embedding")
    ...
    def start(self):
        ...
        for run_number in range(nb_runs):
            ...
            self.build_model_classifier(nb_features,run_number)
